...
def tfidf_w2v(col, word_to_idf_score):
    vecs1 = []
...
def vectorize_question(col, word_to_idf_score):
    new_df = pd.DataFrame()
    new_df['q'] = tfidf_w2v(col, word_to_idf_score)
    new_df[[i for i in range(96)]] = pd.DataFrame(new_df['q'].values.tolist(), index = new_df.index)
    new_df.drop(['q'], axis =1,inplace = True)

    return new_df

def get_TFIDF_W2V(x_train, x_test, word_to_idf_score):

    if os.path.isfile("tfidf_w2v_q1_train.csv") and os.path.isfile("tfidf_w2v_q2_train.csv"):
        tfidf_w2v_q1_train = pd.read_csv("tfidf_w2v_q1_train.csv")
        tfidf_w2v_q2_train = pd.read_csv("tfidf_w2v_q2_train.csv")
    else:
        print('\nTraining data is Vectorizing... \n')
        tfidf_w2v_q1_train = vectorize_question(x_train['question1'], word_to_idf_score)
        tfidf_w2v_q2_train = vectorize_question(x_train['question2'], word_to_idf_score)
    ...
    if os.path.isfile("tfidf_w2v_q1_test.csv") and os.path.isfile("tfidf_w2v_q2_test.csv"):
        tfidf_w2v_q1_test = pd.read_csv("tfidf_w2v_q1_test.csv")
        tfidf_w2v_q2_test = pd.read_csv("tfidf_w2v_q2_test.csv")
    else:
        print('\nTesting data is Vectorizing...\n')
        tfidf_w2v_q1_test = vectorize_question(x_test['question1'], word_to_idf_score)
        tfidf_w2v_q2_test = vectorize_question(x_test['question2'], word_to_idf_score)
    #     saving Vectrized features for future use
...
def get_TFIDF_W2V_quora_test(df):
    if os.path.isfile("quroa_test_tfidf_w2v_q1.csv") and os.path.isfile("quroa_test_tfidf_w2v_q2.csv"):
        tfidf_w2v_q1_test = pd.read_csv("quroa_test_tfidf_w2v_q1.csv")
        tfidf_w2v_q2_test = pd.read_csv("quroa_test_tfidf_w2v_q2.csv")
    else:
        print('\nTesting data is Vectorizing... \n')
        tfidf_w2v_q1_test = vectorize_question(df['question1'])
        tfidf_w2v_q2_test = vectorize_question(df['question2'])
        tfidf_w2v_q1_train.to_csv("quroa_test_tfidf_w2v_q1.csv", index=False)
        tfidf_w2v_q2_test.to_csv("quroa_test_tfidf_w2v_q1.csv", index=False)


    return tfidf_w2v_q1_test, tfidf_w2v_q2_test
